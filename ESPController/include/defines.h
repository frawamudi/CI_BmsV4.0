...
// generator rule activation
#define RULE_13

#define MY_VERSION_1_0 
//#define CURRENT_ACS712
...
#ifdef RULE_13
// RELAY_RULES 12 before mudi they it to 13 
// to make accomadation for 
...
{
  float currentM;
  const float vcc    = 3.30;// supply voltage 5V or 3.3V
  float voltage;
  // added this for calibraion of the senor
    float raw_voltage;
    float compensate_analogue_read;
    #ifdef CURRENT_ACS712
    const float factor = 0.100;
    #else
    const float factor = 0.02;
    #endif

    bool generator_has_started = false;
};

extern version_1_0 verOneParam;  
#endif
