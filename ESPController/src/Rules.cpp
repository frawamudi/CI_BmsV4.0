...
*/
    ...
    #ifdef RULE_13
        //Logic for monitoring genstart Rules
    if (lowestPackVoltage < value[Rule::GenStart] && rule_outcome[Rule::GenStart] == false)
    {
        //Rule - Generator starting voltage (mV)
        rule_outcome[Rule::GenStart] = false;
    }
    else if (lowestPackVoltage > hysteresisvalue[Rule::GenStart] && rule_outcome[Rule::GenStart] == true)
    {
        //Rule - Pack over voltage (mV) - HYSTERESIS RESET
        rule_outcome[Rule::GenStart] = false;
    }
