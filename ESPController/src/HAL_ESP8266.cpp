...
void HAL_ESP8266::ConfigurePins()
{
    ...
    //by default MY_IO_PIN is for input
    #ifdef MY_VERSION_1_0
        #ifdef FOR_OUTPUT
        pinMode(MY_IO_PIN, OUTPUT);
        #else
        pinMode(MY_IO_PIN, INPUT_PULLUP);
        #endif     
    #endif
...
#if defined MY_VERSION_1_0
void HAL_ESP8266::WriteToPortExpander(uint8_t testID, uint8_t state)
{
   //previousRelayState[y] = mysettings.rulerelaydefault[y];
   //Test so we wont mess with Relay output on pin 4,5,6, and 7
    if(testID < 4 && testID == 7) 
        return;
    //LOW is ON
    pcf8574->write(testID, state);
    
   
}

/*float HAL_ESP8266::ReadCurrentSensor()
{
