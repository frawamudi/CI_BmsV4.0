...
{
  ""
};
*/
...
//Current struct declearation
#ifdef MY_VERSION_1_0
version_1_0 verOneParam;
#endif
  ...
  if (!rules.rule_outcome[Rule::BMSError])
  {
    ...
    for (uint8_t i = mqttStartModule; i < TotalNumberOfCells(); i++)
    {
      ...
      if (cmi[i].valid)
      {
        ...
        //doc["current"] = cmi[i].OverAllCurrent;
        serializeJson(doc, jsonbuffer, sizeof(jsonbuffer));
...
// Genstarting default setting
#ifdef RULE_13
  mysettings.rulevalue[Rule::GenStart] = 3000 * 8;
#endif
...
/*void IO_Task()
{
  ...
  if (ControlState == ControllerState::Running)
    { 
      if (rules.lowestPackVoltage < 49000)// Start the generator
        {
          verOneParam.generator_has_started = true;
            //Rule - Generator starting voltage (mV)
            hal.WriteToPortExpander( 7 , LOW);  
            //SERIAL_DEBUG.println("low battery start gen");
            //SERIAL_DEBUG.println(rules.lowestPackVoltage);
            //rule_outcome[Rule::GenStart] = true;
        }
        if (rules.lowestPackVoltage == 54000) // Stop generator
        {
          verOneParam.generator_has_started = false;
            //Rule - Pack over voltage (mV) - HYSTERESIS RESET
            //rule_outcome[Rule::GenStart] = false;
            hal.WriteToPortExpander( 7 , HIGH);
            //SERIAL_DEBUG.println("stop gen");
            //SERIAL_DEBUG.println(rules.lowestPackVoltage);
        }
        if (rules.lowestPackVoltage > 49000 && verOneParam.generator_has_started)
        {
          // leave generator on to contiune charging
            hal.WriteToPortExpander( 7 , LOW);
            //SERIAL_DEBUG.println("gen started and charging");
        }
        else
        {
            hal.WriteToPortExpander( 7 , HIGH);
            //SERIAL_DEBUG.println("leave gen off");
        }
        //hal.WriteToPortExpander( 7 , HIGH);
  }
}*/
...
void setup()
{
  ...
  resetAllRules(); // i intentionally commented the code out for stability during power up
  ...
  else
  {
    ...
    // ACS712 current sensor calaibration
    delay(500);

    /*
  float cal = analogRead(CURRENT_SENSENING);

  if(512 - cal == 0)
  {
    verOneParam.compensate_analogue_read = 0;
  }
  else if(512 > cal)
   {
    verOneParam.compensate_analogue_read = 512 - cal;
   }
  else
   {
    verOneParam.compensate_analogue_read = cal - 512;
   }
*/
    ...
    //I/O user defined task
    //myLazyTimer.attach(10, IO_Task);

    //This handles current reading
    //myCurrentSensingTimer.attach(1, currentReadingCallback);
...
void loop()
{
  ...
  // i added this fuction for controlling the i/o port
...
#ifdef MY_VERSION_1_0
    if (currentMillis - currentReadtimer > 5000)
    {

      //float d = analogRead( CURRENT_SENSENING );
      //SERIAL_DEBUG.println(rules.lowestPackVoltage);
      ...
      {
        verOneParam.voltage = (3.30 / 1023.0) * (analogRead(CURRENT_SENSENING) + verOneParam.compensate_analogue_read); // Read the voltage from sensor
        verOneParam.voltage = verOneParam.voltage - (verOneParam.vcc * 0.5) + 0.007;                                    // 0.007 is a value to make voltage zero when there is no current

        current[i] = verOneParam.voltage / verOneParam.factor;
      }
      ...
      verOneParam.currentM = (sum / 15);

      //verOneParam.voltage =   (3.30 / 1023.0)* (analogRead(CURRENT_SENSENING));
      //+ verOneParam.compensate_analogue_read);// Read the voltage from sensor
      ///SERIAL_DEBUG.print((3.30 / 1023.0)* analogRead(CURRENT_SENSENING));

      //verOneParam.voltage =  verOneParam.voltage - (verOneParam.vcc * 0.5) + 0.007 ;// 0.007 is a value to make voltage zero when there is no current
      //verOneParam.currentM = verOneParam.voltage / verOneParam.factor;

      //SERIAL_DEBUG.print(analogRead( CURRENT_SENSENING ));
      //SERIAL_DEBUG.print("V: ");
      //SERIAL_DEBUG.print(verOneParam.voltage,3);
      //SERIAL_DEBUG.print("V, I: ");
      //SERIAL_DEBUG.print(verOneParam.currentM,2); SERIAL_DEBUG.println("A");

      //SERIAL_DEBUG.print("ADC Value = ");
      //SERIAL_DEBUG.println(verOneParam.currentM);
      currentReadtimer = currentMillis;
    }
#endif
  }
